...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  weights = tf.Variable(
      tf.truncated_normal([7, 7, NUM_CHANNELS, NUM_HIDDEN],  
                          stddev=0.1,
                          seed=SEED))
  bias_h = tf.Variable(tf.zeros([NUM_HIDDEN]))
  bias_v = tf.Variable(tf.constant(0.1, shape=[NUM_CHANNELS]))


  recon = tf.Variable(tf.zeros([BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS]))
  hidden = tf.Variable(tf.zeros([BATCH_SIZE,HIDDEN_DIM,HIDDEN_DIM,NUM_HIDDEN]))
  
  E_p = tf.Variable(tf.Zeros([7, 7, NUM_CHANNELS, NUM_HIDDEN]))

  def v_h(v,sample=True):
	hidden = tf.nn.conv2d(v,
			weights,
			strides=[1,3,3,1],
			padding='VALID')
	if sample:
		thresh = tf.random_uniform([BATCH_SIZE,HIDDEN_DIM,HIDDEN_DIM,NUM_HIDDEN])
		hidden = hidden > thresh
	return hidden

	
  def h_v(h,sample=True):
	recon = tf.nn.conv2d(h,
			tf.reverse(weights,[False,True,True,False]),
			strides=[1,3,3,1],
			padding='VALID')
	if sample:
		thresh = tf.random_uniform([BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS])
		recon = recon > thresh
	return recon
